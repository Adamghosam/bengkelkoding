...
@if ($existingJanji)
  <div class="alert alert-info">
    Anda sudah memiliki janji dengan dokter <strong>{{ $existingJanji->dokter->nama }}</strong>
...
<table class="table table-bordered">
  ...
  <small class="text-muted">(@php echo $periksa->obats->count(); @endphp obat)</small>
                                        ...
                                        <td><span class="badge badge-primary">{{ ucfirst($periksa->status) }}</span></td>
                                        <!-- <td>
                                            @if($periksa->status == 'belum diperiksa')
                                                <form action="{{ route('periksa.updateStatus', $periksa->id) }}" method="POST">
                                                    @csrf
                                                    @method('PATCH')
                                ...
                                @empty
                                    <tr>
                                        <td colspan="6">Belum ada janji dibuat.</td>
                                    </tr>
                                @endforelse
                            </tbody>
                        </table>
